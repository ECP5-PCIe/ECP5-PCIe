...
class LatticeECP5PCIeSERDES(Elaboratable):
    def __init__(self):
        ...
        self.lane = PCIeSERDESInterface(ratio=1)
    ...
    def elaborate(self, platform: Platform) -> Module:
        ...
        if self.gearing == 1:
            m.d.comb += [
                lane.rx_symbol.eq(self.rxbus[0:9]),
                lane.rx_valid.eq(self.rxbus[0:9] != K(14,7)), # SERDES outputs K14.7 when there are coding errors
                self.txbus.eq(Cat(lane.tx_symbol, lane.tx_set_disp, lane.tx_disp, lane.tx_e_idle))
            ]
        else:
            m.d.comb += [
