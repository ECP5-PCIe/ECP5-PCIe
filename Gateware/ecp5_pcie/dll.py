...
class PCIeDLL(Elaboratable): # Based on Yumewatary phy.py
	...
	def elaborate(self, platform: Platform) -> Module:
		...
		with m.FSM(domain="rx") as x:
			with m.State(State.DL_Inactive):
				m.d.rx += self.debug_state.eq(State.DL_Inactive)
				m.d.rx += [
					self.up.eq(0),
					got_p.eq(0),
					got_np.eq(0),
			...
			with m.State(State.DL_Init_FC1):
				m.d.rx += self.debug_state.eq(State.DL_Init_FC1)
				with m.If(~self.ltssm.status.link.up):
			...
			with m.State(State.DL_Init_FC2):
				m.d.rx += self.debug_state.eq(State.DL_Init_FC2)
			...
			with m.State(State.DL_Active):
				m.d.rx += self.debug_state.eq(State.DL_Active)
