...
class LatticeECP5PCIeSERDES(Elaboratable): # Based on Yumewatari
	...
	def elaborate(self, platform: Platform) -> Module:
		...
		tx_elec_idle = Signal() # Set transmitter to Idle, override
		...
		if self.gearing == 1: # Different gearing compatibility!
			...
			m.d.comb += [
				...
				self.tx_bus.eq(Cat(lane.tx_symbol[0:9], lane.tx_set_disp[0], lane.tx_disp[0], lane.tx_e_idle[0] | tx_elec_idle))
			]
		else:
			...
			m.d.comb += [
				...
				self.tx_bus.eq(Cat(
					lane.tx_symbol[0: 9], lane.tx_set_disp[0], lane.tx_disp[0], lane.tx_e_idle[0] | tx_elec_idle,
					lane.tx_symbol[9:18], lane.tx_set_disp[1], lane.tx_disp[1], lane.tx_e_idle[1] | tx_elec_idle)),
			]
...
#            FFSynchronizer(self.tx_bus, tx_bus_s, o_domain="rx"),
		...
		# TN1261 Page 31
		# tx_elec_idle -> 1
		# Wait 120 ns = 30 // gearing
		# pcie_det_en -> 1
		# Wait 120 ns
		# pcie_ct -> 1
		# Wait 4 symbol times (or 4 byte / word clock cycles?)
		# pcie_ct -> 0
		# Wait until pcie_done_s == 0 or a few clock cycles
		# Wait until pcie_done_s == 1 / detect 0 -> 1 transition
		...
		with m.FSM(domain = "tx", reset = "START"):
			...
			with m.State("WAIT-IDLE"):
				with m.If(det_timer == 0):
					m.d.tx += pcie_det_en.eq(1)
					m.d.tx += det_timer.eq(det_delay)
					m.next = "DET-IDLE"
			
			with m.State("DET-IDLE"):
				with m.If(det_timer == 0):
					m.d.tx += pcie_ct.eq(1)
					m.d.tx += det_timer.eq(1 if self.gearing == 2 else 3)
					m.next = "CT-WAIT"
			
			with m.State("CT-WAIT"):
				with m.If(det_timer == 0):
					m.d.tx += pcie_ct.eq(0)
					with m.If(pcie_done_s == 0):
						m.next = "DET-WAIT"
			
			with m.State("DET-WAIT"):
				# TODO: Add a timer here for extra safety in case of glitches?
				with m.If(pcie_done_s == 1):
					m.d.tx += lane.det_status.eq(pcie_con_s)
					m.d.tx += tx_elec_idle.eq(0)
					m.d.tx += det_timer.eq(det_delay)
					m.next = "DET-DONE"
		...
		#        #for i in range(len(lane.tx_e_idle)):
		#        #    m.d.tx += lane.tx_e_idle[i].eq(1)
...
#
		...
		ch_config = {
			...
			**{"o_CHx_FF_TX_D_%d" % n :self.tx_bus[n] for n in range(self.tx_bus.width)}, # Connect TX SERDES inputs to the signals
			...
			# CHx DET
			# "i_CHx_FFC_TX_IDLE"       :tx_elec_idle, # Electrical Idle, not in prjtrellis, use bit 11 and 23 of tx_data instead
			"i_CHx_FFC_PCIE_DET_EN"   :pcie_det_en,  # Enable detect
			"i_CHx_FFC_PCIE_CT"       :pcie_ct,      # Request receiver detection
			"o_CHx_FFS_PCIE_DONE"     :pcie_done,    # Detection complete
			"o_CHx_FFS_PCIE_CON"      :pcie_con,     # Receiver detection result
