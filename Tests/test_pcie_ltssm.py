...
from ecp5_pcie.utils.utils import UARTDebugger, UARTDebugger2, UARTDebugger3
...
class SERDESTestbench(Elaboratable):
	...
	def elaborate(self, platform):
		...
		#m.submodules.serdes = serdes = LatticeECP5PCIeSERDESx4(speed_5GTps=True) # Declare SERDES module with 1:2 gearingLatticeECP5PCIeSERDESx4
		m.submodules.serdes = serdes = LatticeECP5PCIeSERDESx4(speed_5GTps=False, clkfreq=200e6, fabric_clk=False) # Declare SERDES module with 1:2 gearingLatticeECP5PCIeSERDESx4
		m.submodules.aligner = aligner = DomainRenamer("rx")(PCIeSERDESAligner(serdes.lane)) # Aligner for aligning COM symbols
		m.submodules.phy = phy = PCIePhy(aligner, support_5GTps=False)
		...
		m.d.comb += leds[2].eq(~serdes.lane.rx_aligned)
		m.d.comb += leds[3].eq(serdes.lane.rx_locked)
		m.d.comb += leds[4].eq(serdes.lane.rx_present)
		m.d.comb += leds[5].eq(serdes.lane.rx_aligned)
		m.d.comb += leds[7].eq(div[24])
		...
		if self.tstest:
			...
			stp_cnt = Signal(16)
			ack_nak_cnt = Signal(16)
			m.d.comb += stuff_counter.eq(Cat(aligner.debug, phy.descrambled_lane.enable, phy.ltssm.status.link.scrambling, phy.tx.sending_ts, phy.ltssm.status.link.up))#Cat(stp_cnt, ack_nak_cnt))

			with m.If((phy.descrambled_lane.rx_symbol[0:9] == Ctrl.STP) & ltssm.debug_state == TESTING_STATE):
				m.d.rx += stp_cnt.eq(stp_cnt + 1)
			
			with m.If(phy.dll.received_ack_nak):
				m.d.rx += ack_nak_cnt.eq(ack_nak_cnt + 1)
			...
			do_log = Signal()
			#with m.If((phy.descrambled_lane.rx_symbol != 0) & (phy.descrambled_lane.rx_symbol[0:9] != Ctrl.COM) & (time_since_state > 32)):
			#with m.If(phy.descrambled_lane.rx_symbol[0:9] == Ctrl.STP):
			#    m.d.rx += do_log.eq(1)
			...
			debug = UARTDebugger3(uart, 18, CAPTURE_DEPTH, Cat(
				time_since_state,
				lane.rx_symbol, lane.tx_symbol,
				time_since_state[0], time_since_state[1], Signal(6), stuff_counter
				), "rx", (ltssm.debug_state == TESTING_STATE) & do_log)# & (time_since_state < CAPTURE_DEPTH), timeout=100 * 1000 * 1000)

			#debug = UARTDebugger3(uart, 18, CAPTURE_DEPTH, Cat(
			#	time_since_state,
			#	lane.rx_symbol, lane.tx_symbol,
			#	lane.rx_locked & lane.rx_present & lane.rx_aligned, lane.rx_valid[0] & lane.rx_valid[1] & lane.rx_valid[2] & lane.rx_valid[3], Signal(6), stuff_counter
			#	), "rx", (ltssm.debug_state == TESTING_STATE) & do_log)# & (time_since_state < CAPTURE_DEPTH), timeout=100 * 1000 * 1000)
			...
			debug = UARTDebugger2(uart, 25, CAPTURE_DEPTH, Cat(
				...
				phy_rx.ts.link.number, phy_rx.ts.lane.number, phy_rx.ts.link.valid, phy_rx.ts.lane.valid, Signal(1)
				), "rx", (ltssm.debug_state != last_state) & ((ltssm.debug_state >= State.Configuration) | (last_state >= State.Configuration)))
		...
		if arg == "grab":
			...
			for x in range(CAPTURE_DEPTH):
				...
				# Displays symbols received during a state
					...
					print("{:{width}}".format(hex(stuff), width=17), end="\n")
