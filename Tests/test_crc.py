...
if __name__ == "__main__":
	...
	#test_bytes = [0, 0, 4, 0, 0, 1, 0, 0, 0, 0xf, 1, 0, 0, 0]
	#test_bytes = [0,0, 0,0xfb,0,0,0x74, 0, 0, 1, 0, 0xE2, 0, 0x50, 0, 0, 0, 0, 0, 0, 0, 0, 0xA, 0, 0, 0]
	#test_bytes = [0,0,0x4a, 0, 0, 1, 0, 1, 0x10, 4, 0, 0, 0, 0, 0x34, 0x12, 0x78, 0x56]
	#test_bytes = [0,5,0x4a, 0, 0, 1, 0, 1, 0x10, 4, 0, 0, 0, 0, 0, 0, 0, 0]
	test_bytes = [0, 6, 0x44, 0, 0, 1, 0, 0, 0, 0xf, 1, 0, 0, 4, 0, 0, 0x10, 0]
		...
		def process():
			for i in range(len(test_bytes) // 4 + 4):
				...
				if i == 1:
					yield reset_crc.eq(0)
				crc_value = ((yield crc.output))
				print(i, hex(crc_value), hex((yield in_symbol)))
				yield

				if i == len(test_bytes) // 4 + 3:
					print(hex(crc_value), hex(lcrc(test_bytes)))
					assert crc_value == lcrc(test_bytes)
					print("Test passed! LCRC:", hex(crc_value))
